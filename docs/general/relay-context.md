...
Steer it with **focus** (what the next session tackles) and **delivery**: it returns the brief inline by
default, and writes a file only when you ask for one — hand it a path and that's where the file lands.
